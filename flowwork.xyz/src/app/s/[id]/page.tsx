...
export default function JoinSessionPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="h-screen w-full flex flex-col bg-white px-6 py-8 sm:py-12">
      <div className="flex flex-col items-center justify-center flex-1 gap-y-6">
        <Link href="/">
          <img className="h-24 w-24 sm:h-32 sm:w-32" src="/logo.png" />
        </Link>
        {(networkError || sessionName) && <p className="text-base sm:text-lg text-[#999999] text-center">You&apos;ve been invited to join a Flow Work session</p>}
        ...
        {(networkError || sessionName) ?
          null :
          sessionName === null ?
            <p className="text-base sm:text-lg text-center text-[#999999]">Either this session doesn&apos;t exist or it has been removed.</p>
            :
            null}
        ...
        {(networkError || sessionName) && <p className="text-sm sm:text-base text-[#999999] text-center">or copy-paste the session code:&nbsp;<code className="bg-silver">{params.id}</code></p>}
      </div>
